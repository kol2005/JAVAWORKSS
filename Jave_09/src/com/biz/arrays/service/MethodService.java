...
public class MethodService {
	...
	/*
	 * method 를 정의 할때 (method 코드를 작성할때) void method() : return 문이 있거나 없거나 변수를 선언하는
	 ...
	 * method() : : : 변수를 선언하는 키워드를 변수형(Varriable Type)이라고 하는데 같은 키워드가 method() 정의
	 * 문에 나타나면 return tupe 이라고 한다 이때 이 키워드는 return 문에 포함된 값과 밀접한 관계가 있다 int method()
	 * { return 0;} String method() { return "대한민국"; } float method() { return false
	 * ; } ScoreVO method() { return sVO ; }
	 * 
	 * return 값 ; 문장에서 값의 데이터 타입(date type) 을 method() 정의문에 작성해 준다
	 *
	 * method type 은 double 형인데 return 문에 포함된 값이 "00" 문자열이므로 이 코드는 syntax error가 난다
	...
	// method 의 return type이 String(문자열)형이므로
	...
	// 변수형 (Varriable type)이 int return type이 있어야 한다
