...
public class MethodEx_01 {
	...
	public static void main(String[] args) {
		...
		//method() 를 호출하기 위해서
		//ms 라는 객체를 선언하고, 초기화, 생성한다
		//						생성자 메서드
		//						객체를 초기화, 생성하는데
		//						사용되는 method
		MethodService ms = new MethodService();
		
		// 이 method()는 호출만 할수 있는 method 이다
		//즉, void 형 method 는 호출만 할수 있다
		ms.consoleView();
		
		//이 method() 는 int형 return method 이다
		//return 문에 의해서 값, 변수 등을
		// 호출한 곳으로 되 돌려줄 수 있다
		//호출한 곳에서는 되 돌려 받은 값을 변수에 저장
		//할 수 있다
		int num = ms.numRet();
		...
		// method 가 return 하는 값을 변수에 받으려면 (저장하려면)
		// 반드시 같은 type의 변수를 선언하고
		//값을 받아야 한다
		//strRet() 메서드의 return type 이 String 이므로
		//반드시 String 형 변수에 받아야 한다
		String strRet = ms.strRet();
		
		// numRet() method 의 return type 이 int 형인데
		// String 형 변수에 받으려고 하면
		//	type missmatch 오류가 난다
		// 이때는 값을 문자열로 바꾸어주면 된다
		String strRet1 = ms.numRet() + "";
		
		// String = int return 형 method() + String return 형 method()
