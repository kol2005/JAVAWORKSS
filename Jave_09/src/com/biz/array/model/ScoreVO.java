...
public class ScoreVO {
	...
	/*
	 * return type : int 형
	 * 1. 3과목의 점수를 덧셈하여 intSum 멤버변수에 저장
	 * 2. intSum 멤버변수는 private 으로 제한이 된 상태이다
	 * 		외부에서 과목 총점을 읽을 수 없는 상태이다
	 * 3. 그래서 sum() method가 대신 intSum 변수 값을 외부로 return 해 준다
	 * 4. 외부에서는 sum() method 가 return 해준 값을
	 * 		받아서 필요한 곳에서 처리하면 된다
	 * 5. intSum 멤버변수는 3과목의 점수를 덧셈한 값을
	 * 		가지고 있어야 의미(필요)가 있는 변수이다
	 * 		intSum 멤버변수를 직접 접근하여 값을 읽을 경우
	 * 		과목 점수가 합산되지 않아 0이 저장되어 있을
	 * 		가능성도 있다
	 * 		아주 작은 확률이지만 실제 필요하지 않은 0의 
	 *		값을 읽는 것은 코드 실행과정에서 논리적 오류가 날 수 있다
	 ...
	 *			return 하도록 작성
	 *		다. sum() method 에서는 return 문을 실행하기 전에
	 *			과목점수를 합산하여 intSumdp 저장
	 *
	 * == 정보의 은닉, 캡슐화
	 */
	public int sum() {
	...
	/*
	 * ScoreVO 에 멤버변수인 
	 * intSum은 입력된 과목점수에 의해
	 * 계산되어서 값을 보관하는 변수이다.
	 * intSum을 계산하기 위해 sum() 메서드를 생성해 두었다
	 * 
	 * 하지만,
