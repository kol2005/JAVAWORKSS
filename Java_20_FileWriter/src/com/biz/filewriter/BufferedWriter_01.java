...
public class BufferedWriter_01 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// buffer.write() method 로 데이터를 기록하면
			// 메모리의 buffer 영역을 만들고 데이터를 기록한다
			// buffer 메모리 영역이 가득차면
			// JVM 과 OS 는 자동으로 해당 내용을 파일에 기록한다
			buffer.write(str);
			
			// 현재 buffer 에 저장되어 있는 내용을 파일에
			// 강제로 기록하라
			// close()와 차이는 close()를 실행하면
			// 더이상 write() 를 실행할수 없지만,
			// flush() 는 실행한 후에 write() 를 다시 실행해도 된다
			buffer.flush();
			buffer.write("12345");
