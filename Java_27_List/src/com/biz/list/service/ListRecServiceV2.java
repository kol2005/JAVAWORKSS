...
public class ListRecServiceV2 {
	...
	/*
	 * recStrList 는 외부에서 주입될 List
	 * 	- ListRecServiceV2 로 생성한 객체들이 모두 공유하는 List
	 * 	- 다른 객체에서 recStrList 값을(개수를) 변경하면
	 * 	- 모든 객체(인스턴스)가 가지고 있는 recStrList 값이 같이 변동
	 * 
	 * myStrList 는 자체에서 new ArrayList() 로 생성할 List
	 * 	- ListRecServiceV2 로 객체를 생성하면서
	...
	/*
	 * myStrList : ListServiceV1 에서 임의로 생성한 List
	...
	public void remove() {
		...
		// myStrList 에 recStrList 의 0번째 요소를 복사하고
		// 각 플레이어가 받게되는 카드
		this.myStrList.add(this.recStrList.get(0));
		
		// recStrList 에 0번째 요소를 삭제
		// 블랙잭에서 Desk 의 객체
