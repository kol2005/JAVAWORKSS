...
public class ClassEx_01 {
	...
	// main() method 가 있는 클래스의 멤버변수(필드)영역에
	// 변수를 선언
	// static 으로 설정된 method에서
	// 필드변수에 접근을 하려면
	// 필드변수도 static으로 선언을 해야한다
	static String strName = "홍길동";
	
	// main() 는 static method 이며
	 ...
	 * 이때 strName 변수 이름등과는 아무런 상관없이
	 * strName 변수에 담긴 값만 getName() method에 전달된다
	 * 
	 * 전달된 파라메터(값)은
	 * getName() 메서드의 매개변수에 담겨서 
	 * method 내부로 복사된다
	 * getName() 매서드 내부에서 strName 변수의 값을 변경해도
	 * main() method 에서 참조하는 strName 변수의 값은
	 * 아무런 변동이 없다
	 * ==> Call By Value
	 */
