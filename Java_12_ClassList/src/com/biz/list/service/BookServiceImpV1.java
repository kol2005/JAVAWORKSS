...
public class BookServiceImpV1 implements BookService {
	...
	//생성자 메서드
	// scan 과 bookList 를 사용할수 있도록 초기화, 생성
	public BookServiceImpV1() {
		scan = new Scanner(System.in);
		//클래스를 사용하려고 code 를 작성했는데
		// 오류가 난다
		// Ctrl + Shift + O : 자동 import 실행
		bookList = new ArrayList<BookVO>();
	}
	
	// service.input(30); 30개 데이터를 입력 받아라
	...
	public void input() {
		 ...
		 * 
		 * 이 App 에서 NumberFormatException 은 발생할 확률이
		 * 매우(0.00000001%) 높다
		 * 
		 * 이럴 경우 사용자가 입력하는 과정에서
		 * 약간의 실수만 하게 되어도
		 ...
		 * 개발자는 이런 상황에서
		 * 발생할 수 있는 exception 을 예측하고
		 * 적절한 해결방법을 만들어야 한다
		 * =exception handling 이라고 한다
		 * 
		 * 1. 만약 사용자가 가격이나, 출판연도를 문자열을 포함하여
		 * 입력할 경우 안내 메세지를 보여주고, 다음 값을 입력하도록
		 * 유도하는 코드를 작성
		 */
		System.out.print("가격 > ");
		 ...
		 * Run time error 가 발생할 확률이 1/100000000 이라도
		 * 		있다면 그 app 은 사용자에게 상당한 스트레스를 줄수 있다
		 * 
