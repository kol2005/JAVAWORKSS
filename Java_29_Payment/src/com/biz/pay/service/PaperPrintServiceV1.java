...
/*
 * PayReadService 에서 pay.txt 파일을 읽은 정보를 가져오고
 * => payStrList
 * 읽은 정보(리스트)를 한사람분의 급여를 각각 PaperMakeService에
 * 주입을 하고 화폐매수를 계산한 결과를 가져와서
 * console 에 보여줄것
 */
public class PaperPrintServiceV1 {
	...
	public PaperPrintServiceV1(String payFile) {
		// 기본생성자를 호출해서
		// pr 객체와 pm 객체를 생성하고 인스턴스는 만들어 둔다
		this();// 기본생성자를 호출하라
	...
	public void makePaperList() throws Exception {
		// pay.txt 파일을 읽어서 payStrList 로 생성하는 과정
		pr.readPay(this.payFile);
		List<String> payStrList = pr.getPayStrList();
		paperAllList = new ArrayList<List<PaperVO>>();
		
		// payStrList 는 문자열(String)을 담고있는 List 형이기 때문에
		// for 반복문을 이용해서 한개씩 추출이 된다
		for(String pay : payStrList) {
			// pay 사번:급여 형식의 문자열이 들어있다
			// pay를 사번, 급여 부분으로 분해
			String[] pays = pay.split(":");
			// pays[0] 사번
			// pays[1] 급여액
			
			// 문자열로 되어있는 급여액을 정수형으로 변환
