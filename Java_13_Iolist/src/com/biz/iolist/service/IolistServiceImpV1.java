...
public class IolistServiceImpV1 implements IolistService {
	 ...
	 * private : 나만(현재 클래스내에 있는 method 에서만)
	 * 			접근 가능한 친구들
	 * protected : private 성질을 가지면서 현재 클래스를
	 * 				상속한 곳에서는 자유롭게 접근할수 있도록
	 * 				개방하는 접근자
	 * 
	 * IolistEx_01 이나 IolistEx_02 에서는 접근 불가!!
	 * IolistServiceimpV2 는 ImpV1 을 상속 받았으므로
	 * 		접근가능
	 */
	...
	// 자동으로 실행되는 method
	// 1. 멤버변수나 객체등 작동하는데 필요한 요소들을
	//		초기화 하는 코드를 작성
	public IolistServiceImpV1() {
		
		// 이 코드가 누락되면
		// input() method 에서 NullpointException 이 발생한다
	...
	/*
	 * iolist 에 저장된 VO 들의 total 값을 계산
	 */
	@Override
	public void total() {
		...
		for(int i = 0 ; i < nSize ; i++) {
			...
			// vo 변수(객체)에는 iolist 의 i 번째 vo 의 주소가
			// 복사된다
			IolistVO vo = iolist.get(i);
			
			// iolist.get(i).intPrice 읽고
			// iolist.get(i).intQty 를 읽어서 덧셈한후
			// total 에 잠시 보관
			int total = vo.getIntPrice()*vo.getIntQty();
			// iolist 의 i 번째에 보관중인 iolistVO 의
			// intTotal (자동으로) 변수를 찾아가서
			// 값을 저장해 준다
			vo.setIntTotal(total);
		}
		
		// vo 는 iolist 개수만큼 자동 반복 실행 되면서
		// iolist 에서 순서대로 iolistVO 를 하나씩 꺼내서
		// 그 주소를 vo에 담아주고
		// 위의 for() 명령문과 같은 형식의 코드가 실행된다
		for(IolistVO vo : iolist) {
			int total = vo.getIntPrice()*vo.getIntQty();
			
			// iolist 현재 위치에 있는 iolistVO 의 total 변수를
			// 자동으로 찾아가서 값을 세팅해준다
			vo.setIntTotal(total);
		}
