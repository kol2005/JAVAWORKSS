...
public class StudentEx_02 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 그 결과(stdVO)dp toString() 메서드를 실행하라
			// class 의 chainning 코딩
			System.out.println(ss.getStudent("A002").toString());
			
			// stdVO 객체를 받고
			// stdVO 의 toString() method 를 실행하는 코드를
			stdVO = ss.getStudent("A010");
			System.out.println(stdVO.toString());
			// class channing 을 이용해서 한줄로 표현
			System.out.println(ss.getStudent("A010").toString());
			
			// A020 코드가 없으면 null 값을 표시
			System.out.println(ss.getStudent("A020"));
			// 해당 key 값의 데이터(StudentVO)가 없을 경우
			// null 이라고 표시가 되거나,
			// exception 이 발생할 수 있기 때문에
			// 미리 값이 정상적인지 검사하여
			// 그에 따른 적절한 처리를 수행한다
