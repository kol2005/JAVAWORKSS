...
public class GradeScoreService {
	 ...
	 * GradeScoreService 클래스내의 method 에서만
	 * 읽고, 쓰기를 하고 있다
	 * 
	 * 만약
	 * GSEx_03에서 gradeScoreVO 멤버변수를 읽기, 쓰기를
	 * 수행한다면 당연히 이 변수는 public 키워드로 시작을 해야한다
	 * 
	 * 그런데 현재 코드상에서
	 * GradeScoreService 가 아닌곳에서
	 * gradeScoreVO 멤버변수를 접근하지 않는다.
	 * 
	 * 이럴때는 아예 다른 곳에서 gradeScoreVO 멤버변수를
	 * 참조, 접근하지 못하도록 해도 된다
	 * 이제 gradeScoreVO 변수는 
	 * 다른 곳에서 참조하지 못하도록 키워드를 
	 * private로 설정을 한다
	 * 
	 * private : public 은 누구나 접근, 참조 할수 있다는 의미
	 * 			private 는 현재 클래스내의 메서드에서만
